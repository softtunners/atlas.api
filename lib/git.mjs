...
export async function gitCommit(payload = {}) {
  ...
  const added = await git(root, ["add", "--", ...written]);
  if (!added.ok) {
    /* A repository that ignores `.atlas.api` — some people add it while
       treating it as scratch state — fails here with a hint about `-f`. That
       is not advice worth passing on: forcing it once leaves the directory
       ignored and every later commit surprising. Say what to change instead. */
    if (/ignored by one of your \.gitignore/i.test(added.error)) {
      return {
        ok: false,
        error: `${ATLAS_DIR}/ is listed in this repository's .gitignore, so it cannot be committed. Remove that line to keep your API description in the repository.`,
      };
    }
  ...
  if (staged.ok && staged.out === "") {
    return { ok: true, committed: false, pushed: false, files: written, note: "Already up to date." };
  }

  const message = String(payload.message ?? "").trim() || "Update API collection";
  const committed = await git(root, ["commit", "-m", message, "--", ...written]);
  if (!committed.ok) return { ok: false, error: committed.error };

  if (payload.push === false) {
    return { ok: true, committed: true, pushed: false, files: written };
  }
  ...
  if (!pushed.ok) {
    ...
    return { ok: true, committed: true, pushed: false, files: written, error: pushed.error };
  }

  return { ok: true, committed: true, pushed: true, files: written, branch: head.ok ? head.out : null };
}
