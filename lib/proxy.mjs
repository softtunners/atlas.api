...
/**
 * Rebuilds a multipart body from the description the app sends.
 *
 * A file cannot travel over a WebSocket as a `File`, so the app encodes each
 * part and this reassembles it. The boundary is left to `fetch`, which is why
 * no `content-type` is set for these — setting one by hand omits the boundary
 * and the server sees a malformed body.
 */
...
export async function proxy(payload) {
  ...
  const multipart = Array.isArray(payload.multipart) && payload.multipart.length > 0;

  const headers = {};
  for (const [key, value] of Object.entries(payload.headers ?? {})) {
    const name = key.toLowerCase();
    if (BLOCKED.has(name)) continue;
    /* `fetch` writes this itself for a FormData body, boundary included. */
    if (multipart && name === "content-type") continue;
    headers[key] = value;
  }
  ...
  try {
    const response = await fetch(target, {
      ...
      headers,
      body: multipart ? toFormData(payload.multipart) : (payload.body ?? undefined),
      redirect: "follow",
